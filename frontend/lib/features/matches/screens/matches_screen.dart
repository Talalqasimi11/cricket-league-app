...
class _MatchesScreenState extends State<MatchesScreen> {
  ...
  Future<void> _fetchMatches() async {
    ...
    try {
      final resp = await http.get(Uri.parse('${ApiClient.baseUrl}/api/tournament-matches'));
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Wrap(
                    spacing: 8,
                    children: [
                      for (final f in const ['All', 'Live', 'Finished', 'Upcoming'])
                        ChoiceChip(
                          label: Text(f),
                          selected: _filter == f,
                  ...
                  ..._filteredMatches().map((m) {
                    ...
                    final dateStr = dateRaw == null || dateRaw.toString().isEmpty
                        ? 'Not scheduled'
  ...
  List<Map<String, dynamic>> _filteredMatches() {
    ...
    return _matches.where((m) => (m['status'] ?? '').toString().toLowerCase() == key).toList();
  }
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        if (subtitle != null && subtitle.isNotEmpty)
                          Text(subtitle, style: const TextStyle(color: Color(0xFF95c6a9), fontSize: 12)),
                    ...
                    return TextButton.icon(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => LiveMatchViewScreen(matchId: parentId.toString()),
                          ),
                        );
                      },
                      style: TextButton.styleFrom(foregroundColor: const Color(0xFF36e27b)),
