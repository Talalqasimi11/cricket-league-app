...
const getMatchScorecard = async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      db.query(
        `SELECT m.id, m.status, m.overs as max_overs, m.match_datetime as match_date, m.venue,
      ...
      db.query(
        `SELECT pms.player_id, p.player_name, p.team_id,
                pms.runs, pms.balls_faced, pms.fours, pms.sixes, COALESCE(pms.is_out, 0) as is_out,
                CASE 
      ...
      db.query(
        `SELECT pms.player_id, p.player_name, p.team_id,
                pms.balls_bowled, pms.runs_conceded, pms.wickets, COALESCE(pms.maiden_overs, 0) as maiden_overs,
                CASE 
    ...
    const scorecard = inningsData.map(inning => {
      ...
      return {
        ...
        overs: inning.overs,
        batting: inningBatting,
        bowling: inningBowling
      };
    });
    ...
    res.json({
      match: {
        ...
        winner_team_name: matchData.winner_name,
        overs: matchData.max_overs,
      },
      scorecard: scorecard
    });
